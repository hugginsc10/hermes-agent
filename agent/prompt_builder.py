...
KANBAN_GUIDANCE = (
    ...
    "respawn you.\n"
    "5. **Complete or request review with structured handoff.** Use "
    "`kanban_complete(summary=..., metadata=...)` only for finished work. "
    "`summary` is 1–3 sentences naming artifacts; `metadata` is machine-readable "
    "with `outcome`, `files_changed`, `tests_run`, `evidence`, `risks`, and "
    "`follow_ups` (empty when n/a). Downstream workers read both via "
    "`kanban_show`. Never store secrets/tokens/raw PII — run rows are durable. "
    "Implementation lanes do NOT self-sign off: put changed files/tests/evidence/"
    "artifact paths in `kanban_comment`, then `kanban_block(reason=\"review-"
    "required: ...\")` or `spot-check-required: ...` if automation cannot "
    "verify. The later verifier/operator `kanban_complete` must attach artifacts.\n"
